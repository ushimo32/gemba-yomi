...
// 対象読者=書き手の定義(すべての判断の基準)
const READER = `読者は日本の牛の現場で働く人。中心は、和牛(黒毛和種・銘柄系統含む)・交雑種(F1)・アンガス交雑など複数品種を扱う肥育牧場の現場スタッフ。繁殖・酪農の観点も含む。
すべての要約・重要度判定・コメント観点は、この読者にとっての価値で判断すること。特定の品種に決め打ちしない。`;
...
## 定期発表の扱い
- 毎月・毎四半期に機械的に公表される定期指標(輸入予測数量、事業平均価格、月次統計など)は、公表そのものでは重要度を「中」とする。
- 要約に「定期発表」である旨を明記する。
- 前回から大きな変化・水準の転換が読み取れる場合のみ「高」とする。

## コメント観点
- 「確認したい」等の感想で終わらせず、日本の肥育現場の具体的な業務(素牛導入・飼料設計・出荷判断・増体管理・衛生対策)のいずれか1つと接続した観点を1文で書く。
- 特定品種に決め打ちしない。品種による違いが論点になる話題(相場・需要・肉質・増体・飼料反応など)では「和牛と交雑種で影響が異なる」等の品種横断の視点を歓迎する。
